...
const firstImg = document.querySelector('.image-box__big img');
...
let images = document.querySelectorAll('.image-box__small img');
...
window.addEventListener('resize', function () {
  if (window.matchMedia('(max-width: 767px)').matches) {
    firstImg.remove();
    images = document.querySelectorAll('.image-box__small img');
